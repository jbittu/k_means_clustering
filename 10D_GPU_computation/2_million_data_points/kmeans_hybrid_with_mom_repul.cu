...
int num_points = 0, max_iter = 10000, K = 4, NUM_THREADS = 4, D = 0;
float *points = NULL;
char input_file[256] = "20D_2mil.csv";
...
void kmeans_gpu(float *points_host, int N, int K, int max_iter, float tol, double *exec_time, int *conv_iter, float alpha, float repulsion_strength) {
    ...
    for (int i = 0; i < K; i++) {
        ...
        used[r] = 1;
        for (int d = 0; d < D; d++)
            centroids[i * D + d] = points_host[r * D + d];
    }
    free(used);
...
int main(int argc, char **argv) {
    ...
    FILE *log = fopen("10d_2mil_mum_rupel_gpu_results.csv", "w");
    ...
    for (int current_K = 2; current_K <= K && current_K <= num_points; current_K *= 2) {
        ...
        kmeans_gpu(points, num_points, current_K, max_iter, 1e-2, &exec_time, &conv_iter, alpha, repulsion_strength);
